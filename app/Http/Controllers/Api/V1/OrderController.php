...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'user_type' => $request->user ? 'required|in:customer,trader' : 'nullable',
            'trader_image' => 'nullable|image|mimes:jpg,jpeg,png|max:2048',
            'card_back' => 'nullable|image|mimes:jpg,jpeg,png|max:2048',
            'card_face' => 'nullable|image|mimes:jpg,jpeg,png|max:2048',
        ]);
        ...
        // Check if authenticated user is selecting trader type
        if($request->user && $request->user_type === 'trader') {
            $user = $request->user;
            
            // If already approved, just proceed
            if ($user->is_trader_approved) {
                $user->user_type = 'trader';
                $user->save();
            } else {
                // 1. Check if documents exist in DB
                $has_docs = !empty($user->trader_image) && !empty($user->card_back) && !empty($user->card_face);
                
                // 2. Check if they are uploading now (must upload all 3 together for the first time)
                $uploading_docs = $request->hasFile('trader_image') && $request->hasFile('card_back') && $request->hasFile('card_face');

                if (!$has_docs && !$uploading_docs) {
                    return response()->json([
                        'errors' => [['code' => 'trader_documents_required', 'message' => translate('messages.trader_documents_required')]],
                        'requires_trader_registration' => true,
                    ], 403);
                }

                // 3. If they are uploading now, save and return "waiting for approval"
                if ($uploading_docs) {
                    $user->trader_image = Helpers::upload('profile/', 'png', $request->file('trader_image'));
                    $user->card_back = Helpers::upload('profile/', 'png', $request->file('card_back'));
                    $user->card_face = Helpers::upload('profile/', 'png', $request->file('card_face'));
                    $user->is_trader_approved = false;
                    $user->user_type = 'trader';
                    $user->save();

                    return response()->json([
                        'errors' => [['code' => 'waiting_for_approval', 'message' => translate('messages.trader_registration_pending_approval')]],
                        'registration_submitted' => true,
                    ], 403);
                }

                // 4. If documents exist but not approved yet
                if (!$user->is_trader_approved) {
                    return response()->json([
                        'errors' => [['code' => 'trader_not_approved', 'message' => translate('messages.trader_is_not_approved_yet')]],
                    ], 403);
                }
            }
        } elseif($request->user && $request->user_type === 'customer') {
    ...
    $digital_payment = Helpers::get_business_settings('digital_payment');
        ...
        if ($request->order_type !== 'parcel') {
           ...
           if ($store->active == 0) {
            ...
            if ($store->open == false && $request->user_type !== 'trader') {
                return response()->json([
                    'errors' => [
                        ['code' => 'order_time', 'message' => translate('messages.store_is_closed_at_order_time')]
                    ]
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        $min_order_qty = (int) (BusinessSetting::where('key', 'min_order_qty_for_trader')->first()?->value ?? 120);
        // سعر الجملة (tier_prices) للتجار المعتمدين فقط - مثل CartController
        $user = $request->user;
                    ...
                    if ($product) {
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            if (count($product_variations)) {
                                ...
                                $price = $isApprovedTrader ? Helpers::calculate_tiered_price($product, $c['quantity']) : $product['price'];
                            }
                            if ($c['quantity'] >= $min_order_qty) {
                                $is_large_order = true;
                            }
                            $product->tax = $store->tax;
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
        ...
        try {
            ...
            if($request->user){
                ...
                // If trader and doesn't have a vendor/store yet, create it now
                if ($request->user_type === 'trader' && !$customer->vendor_id) {
                    $trader_email = $customer->email ?: ($customer->phone ? $customer->phone . '@nasia-ec.com' : 'trader_' . $customer->id . '@nasia-ec.com');
                    
                    $vendor = new Vendor();
                    $vendor->f_name = $customer->f_name;
                    $vendor->l_name = $customer->l_name;
                    $vendor->email = $trader_email;
                    $vendor->phone = $customer->phone;
                    $vendor->password = $customer->password;
                    $vendor->status = 1;
                    $vendor->save();

                    $customer->vendor_id = $vendor->id;

                    $trader_store = new Store();
                    $trader_store->name = $customer->f_name . "'s Store";
                    $trader_store->phone = $customer->phone;
                    $trader_store->email = $trader_email;
                    $trader_store->logo = $customer->trader_image;
                    $trader_store->vendor_id = $vendor->id;
                    $trader_store->latitude = $request->latitude;
                    $trader_store->longitude = $request->longitude;
                    $trader_store->address = $request->address;
                    $trader_store->zone_id = $order->zone_id;
                    $trader_store->module_id = $request->header('moduleId');
                    $trader_store->status = 1;
                    $trader_store->save();

                    StoreLogic::insert_schedule($trader_store->id);
                    
                    if($trader_store->module) {
                        $trader_store->module->increment('stores_count');
                    }
                }
                
                $customer->save();

                // Add products to trader's store
                if ($request->user_type === 'trader' && $customer->vendor_id) {
                    $trader_store = Store::where('vendor_id', $customer->vendor_id)->first();
                    if ($trader_store) {
                        foreach ($carts as $cart) {
                            $product = Item::find($cart['item_id']);
                            if ($product) {
                                // Check if item already exists in trader's store
                                $existing_item = Item::where('store_id', $trader_store->id)
                                    ->where('name', $product->name)
                                    ->first();
                                
                                if (!$existing_item) {
                                    $new_item = $product->replicate();
                                    $new_item->store_id = $trader_store->id;
                                    $new_item->sku = null; // Clear SKU to avoid duplicate
                                    $new_item->order_count = 0;
                                    $new_item->status = 1;
                                    $new_item->is_approved = 1;
                                    $new_item->save();
                                    
                                    // Copy translations
                                    foreach ($product->translations as $translation) {
                                        $new_translation = $translation->replicate();
                                        $new_translation->translationable_id = $new_item->id;
            ...
            try {
                ...
                if(!in_array($order->payment_method, ['digital_payment', 'partial_payment', 'offline_payment'])  || $payments){
                        ...
                        $this->notify_nearby_traders($order);
            ...
            return response()->json([
                ...
                'is_large_order' => $is_large_order && $request->user_type !== 'trader',
            ], 200);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function prescription_place_order(Request $request)
    {
        ...
        // Remove the cash on delivery check
...
//        })
    ...
    private function notify_nearby_traders($order)
    {
        $zone_id = $order->zone_id;
        $latitude = $order->delivery_address ? json_decode($order->delivery_address, true)['latitude'] : null;
        $longitude = $order->delivery_address ? json_decode($order->delivery_address, true)['longitude'] : null;

        if (!$latitude || !$longitude) {
            return;
        }

        $trader_stores = Store::where('zone_id', $zone_id)
            ->whereHas('vendor', function ($q) {
                $q->whereHas('users', function ($u) {
                    $u->where('user_type', 'trader');
                });
            })
            ->Active()
            ->get();
        ...
        foreach ($trader_stores as $store) {
            $can_fulfill = true;
            foreach ($order->details as $detail) {
                $order_item_details = json_decode($detail->item_details, true);
                if (!isset($order_item_details['name'])) continue;
                $needed_name = $order_item_details['name'];
                $needed_qty = $detail->quantity;

                $trader_item = Item::where('store_id', $store->id)
                    ->where('name', $needed_name)
                    ->first();

                if (!$trader_item || $trader_item->stock < $needed_qty) {
                    $can_fulfill = false;
                    break;
                }
            }
            ...
            if ($can_fulfill) {
                // Assuming vendor user is linked via vendor_id on User model or we go through vendor relationship
                // The relationship is Vendor hasMany User (users).
                $trader_user = $store->vendor->users()->where('user_type', 'trader')->first();

                if ($trader_user && $trader_user->cm_firebase_token) {
                    $data = [
                        'title' => translate('messages.new_order_available'),
                        'description' => translate('messages.you_can_fulfill_order_nearby'),
                        'order_id' => $order->id,
                        'image' => '',
                        'type' => 'trader_order_request', 
                    ];
                    Helpers::send_push_notif_to_device($trader_user->cm_firebase_token, $data);
