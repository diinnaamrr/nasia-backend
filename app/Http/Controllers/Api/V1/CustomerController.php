...
class CustomerController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
        ...
        if($request->button_type != 'change_password' && !$request->otp ){
            $normalized_request_phone = Helpers::normalizePhone($request->phone);
            if (  data_get($login_settings, 'phone_verification_status') == 1  && ($user->phone != $normalized_request_phone  || $request->button_type == 'phone' || (!$user->is_phone_verified  && !$request->button_type) )) {
                if (data_get($login_settings, 'firebase_otp_verification') == 1) {
                    ...
                    $verification_data =  $this->verification_check($normalized_request_phone);
