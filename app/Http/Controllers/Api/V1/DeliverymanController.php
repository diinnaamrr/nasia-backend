...
class DeliverymanController extends Controller
{
    ...
    public function get_statistics_summary(Request $request)
    {
        $dm = $request->authenticated_delivery_man;

        // Get total delivered orders count
        $total_delivered_orders = Order::withoutGlobalScopes()->where('delivery_man_id', $dm->id)
            ->where('order_status', 'delivered')
            ->count();

        // Get total assigned orders count
        $total_assigned_orders = Order::withoutGlobalScopes()->where('delivery_man_id', $dm->id)
            ->count();

        // Get total earnings (delivery charge + tips)
        $total_earnings = OrderTransaction::where('delivery_man_id', $dm->id)
            ->whereHas('order', function($query) {
                $query->withoutGlobalScopes()->where('order_status', 'delivered');
            })
            ->notRefunded()
            ->sum(\DB::raw('original_delivery_charge + COALESCE(dm_tips, 0)'));

        $avg_earnings = $total_delivered_orders > 0 ? ($total_earnings / $total_delivered_orders) : 0;
        $avg_success = $total_assigned_orders > 0 ? (($total_delivered_orders / $total_assigned_orders) * 100) : 0;

        $data = [
            'total_delivered_orders' => (int) $total_delivered_orders,
    ...
    public function get_weekly_report(Request $request)
    {
        ...
        $orders = Order::withoutGlobalScopes()->where('delivery_man_id', $dm->id)
            ->where('order_status', 'delivered')
            ->whereBetween('delivered', [$request->date_from, $request->date_to . ' 23:59:59'])
            ->with('transaction')
            ->get();
        ...
        $begin = Carbon::parse($request->date_from);
        $end = Carbon::parse($request->date_to);
        
        // Group orders by date for efficient lookup
        $grouped_orders = $orders->groupBy(function($order) {
            return Carbon::parse($order->delivered)->format('Y-m-d');
        });

        // Calculate totals and build daily report for every day in range
        $daily_report = [];
        $total_earnings = 0;
        
        for ($date = $begin; $date->lte($end); $date->addDay()) {
            $formatted_date = $date->format('Y-m-d');
            $day_orders = $grouped_orders->get($formatted_date, collect());
            
            $day_earnings = 0;
            foreach ($day_orders as $order) {
                if ($order->transaction && $order->transaction->status != 'refunded_with_delivery_charge' && $order->transaction->status != 'refunded_without_delivery_charge') {
                    $day_earnings += ($order->transaction->original_delivery_charge ?? 0) + ($order->transaction->dm_tips ?? 0);
                }
            }
            ...
            $daily_report[] = [
                ...
                'date' => $formatted_date,
                'total_order' => $day_orders->count(),
                'total_earnings' => (float) $day_earnings,
            ];
        }
