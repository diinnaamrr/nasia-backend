...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // ================== New: LandingDashboard Departments ==================
        $departments = Department::with(['stockUnits' => function ($q) {
            $q->where('active', true);
        }])->where('visible', true)->orderBy('display_order')->get();

        // ================== New: Flash Sales ==================
        $flashSales = PromoCampaign::where('enabled', true)
            ->where('starts_at', '<=', now())
            ->where('ends_at', '>=', now())
            ->with(['stockUnits' => function ($q) {
                $q->where('active', true);
            }])->get();
        ...
        $landing_data = [
            ...
            // ======= New Added Sections =======
            'landing_departments' => $departments,
    ...
    // ================== Show More AJAX for Departments ==================
    public function getAllProducts($id)
    {
        $department = Department::with(['stockUnits' => function($q){
            $q->where('active', true);
        }])->findOrFail($id);

        $html = '';
        foreach($department->stockUnits as $unit){
            $html .= '<div class="light-glass-card">';
            $html .= '    <div class="showroom-img-box" style="height: 200px;">';
            $html .= '        <img src="'.($unit->thumbnail ? asset("storage/".$unit->thumbnail) : "https://via.placeholder.com/400x300").'">';
            $html .= '    </div>';
            $html .= '    <div class="showroom-details">';
            $html .= '        <h3 class="showroom-p-title" style="font-size: 1.1rem;">'.$unit->title.'</h3>';
            $html .= '        <div class="showroom-price-box">';
            if($unit->base_price > 0 && $unit->base_price > $unit->final_price){
                $html .= '<span class="showroom-price-was" style="font-size: 0.85rem;">'.number_format($unit->base_price,2).' EGP</span>';
            }
            $html .= '            <span class="showroom-price-now">'.number_format($unit->final_price,2).' EGP</span>';
            $html .= '        </div>';
            $html .= '    </div>';
            $html .= '</div>';
        }
