...
class VendorController extends Controller
{
    ...
    public function pending_requests(Request $request)
    {
        ...
        if ($tab == 'trader') {
            $stores = User::where('user_type', 'trader')
                ->where('is_trader_approved', 0)
        ...
        $zone = is_numeric($zone_id)?Zone::findOrFail($zone_id):null;
        return view('admin-views.vendor.pending_requests', compact('stores', 'zone','type', 'search_by', 'tab'));
    }

    public function trader_approval(Request $request)
    {
        $user = User::findOrFail($request->id);
        $user->is_trader_approved = $request->status;
        
        if ($request->status == 0) {
            $user->user_type = 'customer'; // Revert to customer if denied
        }
        
        $user->save();

        if ($request->status == 1) {
            Toastr::success(translate('messages.trader_approved_successfully'));
        } else {
            Toastr::success(translate('messages.trader_request_denied_successfully'));
        }
