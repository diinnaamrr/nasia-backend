...
class Helpers
{
    ...
    public static function calculate_tiered_price($item, $quantity, $base_price = null)
    {
        $price = $base_price ?? $item->price;
        $tier_prices = $item->tier_prices;

        if (is_string($tier_prices)) {
            $tier_prices = json_decode($tier_prices, true);
        }

        if (is_array($tier_prices) && count($tier_prices) > 0) {
                                                        ...
                                                        $selected_addon_quantity,$trans = false, $local = 'en')
    {
        ...
        $data['tier_prices'] = $data['tier_prices'] ? (gettype($data['tier_prices']) == 'array' ? $data['tier_prices'] : json_decode($data['tier_prices'],true)) : [];
    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en' , $temp_product=false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['tier_prices'] = $item['tier_prices'] ? json_decode($item['tier_prices'], true) : [];
            ...
            $data['tier_prices'] = $data['tier_prices'] ? json_decode($data['tier_prices'], true) : [];
            unset($data['pharmacy_item_details']);
            unset($data['store']);
            unset($data['rating']);
            unset($data['nutritions']);
...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function normalizePhone($phone)
    {
        ...
        // Remove spaces or other invisible chars if any remained
        $phone = trim($phone);
        
        // Handle +200... (common mistake) -> +20...
        if (preg_match('/^\+200/', $phone)) {
            $phone = '+20' . substr($phone, 4);
        }
        
        // Handle Egyptian local format 01... -> +201...
        if (preg_match('/^01[0125]/', $phone)) {
            $phone = '+20' . substr($phone, 1);
        }
        
        // If it starts with 20... but no +, add +
        if (preg_match('/^20/', $phone) && !str_starts_with($phone, '+')) {
            $phone = '+' . $phone;
        }
        
        // If it still doesn't have a +, and it's long enough, add +
