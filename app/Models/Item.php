...
{
    // Check if image_full_url is already set, otherwise use image
    $imageFullUrl = $this->attributes['image_full_url'] ?? $this->image;

    // If it's already a full URL, return it as is
    if (filter_var($imageFullUrl, FILTER_VALIDATE_URL)) {
        return $imageFullUrl;
    }

    // If storage exists, construct full URL
    if (!empty($this->storage)) {
        foreach ($this->storage as $storage) {
            if ($storage['key'] == 'image') {
                return Helpers::get_full_url('product', $imageFullUrl, $storage['value']);
            }
        }
    }

    // Default fallback using 'public' storage
    return Helpers::get_full_url('product', $imageFullUrl, 'public');
}
