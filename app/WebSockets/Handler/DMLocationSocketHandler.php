    ...
    function onOpen(ConnectionInterface $conn)
{
    try {
        ...
        // Log or notify that the WebSocket connection was established
        \Log::info('WebSocket connection opened: ' . $conn->socketId);
    } catch (UnknownAppKey $e) {
        // If the appKey is invalid, disconnect the user
        $conn->send(json_encode(['error' => 'Invalid appKey']));
        $conn->close();
        \Log::error('Invalid appKey connection attempt: ' . $e->getMessage());
    }
}
