                            ...
                            <li class="nav-item {{ Request::is('admin/order/list/all') ? 'active' : '' }}">
                                <a class="nav-link" href="{{ route('admin.order.list', ['all']) }}" title="{{ translate('messages.all_orders') }}">
                                    ...
                                    <span class="text-truncate sidebar--badge-container">
                                     ...
                                     <span id="orderCount" class="badge badge-soft-info badge-pill ml-1"
                                    data-count="{{ \App\Models\Order::StoreOrder()->module(Config::get('module.current_module_id'))->count() }}">
                                   {{ \App\Models\Order::StoreOrder()->module(Config::get('module.current_module_id'))->count() }}
                                     </span>

                                            <audio id="orderSound" preload="auto">
                                           <source src="{{ url('public-sounds/notification.mp3') }}" type="audio/mpeg">
                                                </audio>
...
@push('script_2')
<script>
let currentCount = parseInt(document.getElementById('orderCount').dataset.count);

let lastNotifiedCount = localStorage.getItem('lastNotifiedOrderCount');
if (lastNotifiedCount !== null) {
    currentCount = parseInt(lastNotifiedCount);
}

setInterval(function () {
    fetch("{{ url('/admin/order/count') }}")
        .then(res => res.json())
        .then(data => {
            ...
            if (newCount > currentCount) {
                ...
                document.getElementById('orderCount').innerText = newCount;
                document.getElementById('orderCount').dataset.count = newCount;

                currentCount = newCount;
                localStorage.setItem('lastNotifiedOrderCount', newCount);
            }
        });
}, 5000);
</script>
