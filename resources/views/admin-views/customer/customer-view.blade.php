...
@section('content')
    ...
    $totalOrderAmount = $total_order_amount[0]->total_order_amount ?? 0;

    $averageOrderValue = $totalOrders > 0 ? $totalOrderAmount / $totalOrders : 0;
    $averageOrdersPerCustomer = $totalOrders;

    $joinedDate = \Carbon\Carbon::parse($customer->created_at);
    $now = \Carbon\Carbon::now();
    $customerAgeInDays = $joinedDate->diffInDays($now);

    $estimatedCustomerValue = $averageOrderValue * $averageOrdersPerCustomer * $customerAgeInDays;
@endphp
...
<div class="row mb-3 g-2">
        ...
        <div class="card h-100">
            <div class="card-body text-center">
                ...
                <h4 class="text-primary">
                    {{ \App\CentralLogics\Helpers::format_currency($averageOrderValue) }}
                </h4>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card h-100">
            <div class="card-body text-center">
                <h6 class="text-muted">متوسط عدد الطلبات</h6>
                <h4 class="text-primary">{{ $averageOrdersPerCustomer }}</h4>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card h-100">
            <div class="card-body text-center">
                <h6 class="text-muted">متوسط عمر العميل (بالأيام)</h6>
                <h4 class="text-primary">{{ $customerAgeInDays }}</h4>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card h-100">
            <div class="card-body text-center">
                <h6 class="text-muted">القيمة التقديرية للعميل</h6>
                <h4 class="text-success">
                    {{ \App\CentralLogics\Helpers::format_currency($estimatedCustomerValue) }}
                </h4>
            </div>
        </div>
    </div>
