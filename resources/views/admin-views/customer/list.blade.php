                        ...
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th class="border-0">{{ translate('messages.Joining_date') }}</th>
                              	<th class="border-0">القيمة التقديرية للعميل</th>
                        ...
                        <tbody id="set-rows">
                            @foreach ($customers as $key => $customer)
                                ...
                                <tr class="">
                                  ...
                                  @php
    $totalAmount = $customer->orders()->sum('order_amount');
    $orderCount = $customer->orders_count ?? 0;
    $averageOrderValue = $orderCount > 0 ? $totalAmount / $orderCount : 0;

    $customerAgeInDays = \Carbon\Carbon::parse($customer->created_at)->diffInDays(now());
    $estimatedValue = $averageOrderValue * $orderCount * $customerAgeInDays;
@endphp
<td>
    <label class="badge text-success">
        {{ \App\CentralLogics\Helpers::format_currency($estimatedValue) }}
    </label>
</td>
    ...
    <!-- Delete Button -->
    <form action="{{ route('admin.users.customer.delete', [$customer->id]) }}" method="POST" style="display: inline;">
        @csrf
        @method('DELETE')
...
<script>
    ...
    $('.delete-btn').on('click', function () {
        ...
        let form = document.getElementById(`delete-form-${customerId}`);

        Swal.fire({
            title: '{{ translate("messages.Are you sure?") }}',
            text: '{{ translate("messages.This action cannot be undone!") }}',
            icon: 'warning',
            showCancelButton: true,
