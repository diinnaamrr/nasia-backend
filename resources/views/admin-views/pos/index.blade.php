...
@section('content')
    ...
    <form id="search-form" class="search-form" method="GET" action="{{ url()->current() }}">
        <!-- Keep the existing store_id in the URL -->
        <input type="hidden" name="store_id" value="{{ request()->query('store_id', '') }}">

        <!-- Search by Product Name -->
        <div class="position-relative mb-2">
            <input id="datatableSearch" type="search" value="{{ $keyword ?? '' }}" name="keyword" class="form-control h--45px pl-5" placeholder="{{ translate('messages.Search_by_product_name') }}" aria-label="{{ translate('messages.search_here') }}" />

            <img width="16" height="16" src="{{ asset('public/assets/admin/img/icons/search-icon.png') }}" alt="" class="search-icon">

            @if($keyword)
                <button type="reset" class="btn btn--primary ml-2 location-reload-to-base-pos" data-url="{{ url()->current() }}?store_id={{ request()->query('store_id', '') }}">{{ translate('messages.reset') }}</button>
            @endif
        </div>
    </form>
</div>
...
<div class="col-sm-12 col-12">
    <form id="sku-search-form" class="search-form" method="GET" action="{{ url()->current() }}">
        <!-- Keep the existing store_id in the URL -->
        <input type="hidden" name="store_id" value="{{ request()->query('store_id', '') }}">

        <!-- Search by SKU -->
        <div class="position-relative">
            <input id="skuSearch" type="search" value="{{ $sku ?? '' }}" name="sku" class="form-control h--45px pl-5" placeholder="{{ translate('messages.Search_by_SKU') }}" aria-label="{{ translate('messages.search_sku_here') }}" />
            ...
            @endif
                            ...
                            ?>
                                @foreach($products as $product)
                                    @php
    // Check if the image is just a filename (no "http" in it)
    if (strpos($product->image, 'http') === false) {
        $image_url = asset('storage/app/public/product/' . $product->image);
    } else {
        $image_url = $product->image;
    }
@endphp

<div class="order--item-box item-box quick-View" data-id="{{ $product->id }}">
