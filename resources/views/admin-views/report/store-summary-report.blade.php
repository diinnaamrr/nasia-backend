...
<div class="content container-fluid">
    ...
    <img src="{{ asset('/public/assets/admin/img/report/wallet.svg') }}" alt="">
    <div class="info">
        @php
            // عدد كل المستخدمين
            $customer_count = \App\Models\User::count();

            // متوسط قيمة الطلب = إجمالي قيمة الطلبات المكتملة / عدد الطلبات المكتملة
            $average_order_value = $total_delivered > 0 ? $total_order_amount / $total_delivered : 0;

            // متوسط عدد الطلبات لكل مستخدم = عدد الطلبات / عدد المستخدمين
            $average_orders_per_customer = $customer_count > 0 ? $orders->count() / $customer_count : 0;

            // قيمة العميل = متوسط قيمة الطلب × متوسط عدد الطلبات
            $customer_value = $average_order_value * $average_orders_per_customer;
        @endphp

        <h4 class="subtitle">
            {{ \App\CentralLogics\Helpers::number_format_short($customer_value) }}
            {{ \App\CentralLogics\Helpers::currency_symbol() }}
        </h4>
        <h6 class="subtext">
            {{ translate('messages.Customer Value') }}
            <span class="input-label-secondary text--title" data-toggle="tooltip"
                data-placement="right"
                data-original-title="{{ translate('Average order value × average orders per user') }}">
