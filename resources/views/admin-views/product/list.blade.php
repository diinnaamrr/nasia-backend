...
@section('content')
    ...
    <form class="search-form">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="input-group input--group">
                    <input id="skuSearch" name="sku" value="{{ request()?->sku ?? null }}" type="search"
                        class="form-control h--40px" placeholder="{{translate('ابحث ب البار كود')}}" 
                        aria-label="{{translate('messages.search_here')}}">
                    <button type="submit" class="btn btn--secondary h--40px">
                        <i class="tio-search"></i>
                    </button>
    ...
    @if(request()->get('search') || request()->get('sku'))
            ...
            <div class="table-responsive datatable-custom" id="table-div">
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <tr>
                        ...
                        <th class="border-0">{{translate('messages.بار كود')}}</th>
                        ...
                        <th class="border-0 text-center">{{translate('messages.price')}}</th>
                           <th class="border-0">{{translate('production_date')}}</th>
                        <th class="border-0">{{translate('expiration_date')}}</th>
<th id="status-column" style="cursor:pointer">
   {{translate('messages.status')}}
    <span id="status-indicator">(الكل)</span>
</th>
    ...
    <a class="media align-items-center" href="{{ route('admin.item.view', [$item['id']]) }}">
        <img class="avatar avatar-lg mr-3 onerror-image"
             src="{{ filter_var($item['image'], FILTER_VALIDATE_URL) ? $item['image'] : asset('storage/app/public/product/' . $item['image']) }}"
        ...
        <div title="{{ $item['name'] }}" class="media-body">
            <h5 class="text-hover-primary mb-0">{{ Str::limit($item['name'], 20, '...') }}</h5>
                          ...
                          <td>
                <span class="badge badge-soft-info">{{ $item->sku ?? 'N/A' }}</span> 
            </td>
                            ...
                            <td>
                                @if($item['production_date'])
                                    <span class="badge badge-soft-info">{{ \Carbon\Carbon::parse($item['production_date'])->format('Y-m-d') }}</span>
                                @else
                            ...
                            <td>
                                @if($item['expiration_date'])
                                    <span class="badge badge-soft-warning">{{ \Carbon\Carbon::parse($item['expiration_date'])->format('Y-m-d') }}</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (statusColumn) {
        // الحصول على الحالة الحالية من URL
        let urlParams = new URLSearchParams(window.location.search);
        let currentStatus = urlParams.get('status') || 'all';

        // تحديث المؤشر بناء على الحالة الحالية
        updateStatusIndicator(currentStatus);

        statusColumn.addEventListener('click', function () {
            if (currentStatus === 'all') {
                currentStatus = '1'; // المتفعّلين
            } else if (currentStatus === '1') {
                currentStatus = '0'; // الغير متفعّلين
            } else {
                currentStatus = 'all'; // الكل
            }

            updateStatusIndicator(currentStatus);
            
            // نعيد تحميل الصفحة مع باراميتر status
        ...
        function updateStatusIndicator(status) {
            if (statusIndicator) {
                if (status === '1') {
                    statusIndicator.textContent = '(مفعّل)';
                } else if (status === '0') {
                    statusIndicator.textContent = '(غير مفعّل)';
                } else {
                    statusIndicator.textContent = '(الكل)';
                }
            }
        }
    }
