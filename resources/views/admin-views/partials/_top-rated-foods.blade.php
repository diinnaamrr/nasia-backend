                ...
                <a href="{{ route('admin.item.view', [$item['id']]) }}">
                    <div class="rated-media d-flex align-items-center">
                        ...
                        <span class="line--limit-1 w-0 flex-grow-1" title="{{ $item?->name }}">
                            ...
                            <span class="line--limit-1 d-block" title="{{ $item?->name }}">
                                {{ Str::limit($item->name ?? translate('messages.Item deleted!'), 20, '...') }}
                            </span>
                            @if($item['production_date'])
                                <div class="fz--11 text-muted mt-1">
                                    {{ translate('production_date') }}: {{ \Carbon\Carbon::parse($item['production_date'])->format('Y-m-d') }}
                                </div>
                            @endif
                            @if($item['expiration_date'])
                                <div class="fz--11 text-muted">
                                    {{ translate('expiration_date') }}: {{ \Carbon\Carbon::parse($item['expiration_date'])->format('Y-m-d') }}
                                </div>
                            @endif
                        </div>
                        <div>
